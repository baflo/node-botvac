...
Client.prototype.authorize = function (email, password, force, callback) {
    if (!this._token || force) {
        api.request(this._baseUrl + this._apiPaths.auth, {email: email, password: password}, 'POST', null, (function (error, body) {
...
Client.prototype.getRobots = function (callback) {
    if (this._token) {
        api.request(this._baseUrl + this._apiPaths.getRobots, null, 'GET', {Authorization: this._tokenType + this._token}, (function (error, body) {
            if (!error && body) {
                var robots = [];
